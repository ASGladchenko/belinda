...
export default function AdminAside() {
  ...
  const switcherClass = clsx(
    ...
    {
      'translate-x-[-4px] duration-300 lg:translate-x-1/3 right-2 lg:right-0': isNavBar,
      'translate-x-full duration-300 lg:translate-x-1/3 right-[-4px] lg:right-0 ': !isNavBar,
    }
  );

  const headerClass = clsx(
    'flex w-full justify-center transition-all duration-300 mb-4 text-xs font-bold select-none',
    {
      'text-lg ': isNavBar,
    }
  );
    ...
    <aside className={asideClass}>
      <div className={switcherClass} onClick={() => setIsNavBar(!isNavBar)}>
        <Right
          width={16}
          height={16}
          className={`${isNavBar ? 'rotate-180' : ''} transition-transform duration-300`}
        />
      </div>
